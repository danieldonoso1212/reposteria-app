      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-cream-50 to-cream-100">
        <div className="card p-6 sm:p-8 max-w-md w-full text-center">
          ...
          <p className="text-sm text-cream-600 mb-2">
            {carrito.length} producto{carrito.length > 1 ? 's' : ''} · Total: <strong>{formatCOP(totalCarrito)}</strong>
          </p>
                      ...
                      <div key={i} className="flex items-start justify-between bg-cream-50 border border-cream-200 rounded-lg p-3">
                        <div className="flex-1 min-w-0">
                          ...
                          {p.extrasNombres.length > 0 && (
                            <p className="text-xs text-green-700">+ {p.extrasNombres.join(', ')}</p>
                          )}
                          {p.notas && (
                ...
                <div className="space-y-3">
                  ...
                  {precioBaseActual > 0 && (
                    <div className="text-right text-sm text-cream-700">
                      Subtotal: <strong>{formatCOP(precioBaseActual + precioExtrasActual)}</strong>
                    </div>
                  )}
